...
describe.each(['/', '\\'])('FileUtils with separator %s', separator => {
  function normalizeName(name: string): string {
    return name.replace(/\//g, separator);
  }

  it('gets extension', () => {
    function testName(name: string, expectedExtension: string) {
      expect(FileUtils.getExtension(normalizeName(name), separator)).toBe(
        expectedExtension
      );
    }
    testName('noext', '');
  ...
  it('gets the path', () => {
    function testName(name: string, expectedPath: string) {
      expect(FileUtils.getPath(normalizeName(name), separator)).toBe(
        normalizeName(expectedPath)
      );
    }

    function testError(name: string) {
      expect(() => FileUtils.getPath(name, separator)).toThrow();
    }
  ...
  it('makePath handles names with or without trailing slash', () => {
    ...
    const DEFAULT_PATH = `${DEFAULT_DIR}${separator}`;
    expect(FileUtils.makePath(DEFAULT_DIR, separator)).toBe(DEFAULT_PATH);
    expect(FileUtils.makePath(DEFAULT_PATH, separator)).toBe(DEFAULT_PATH);
  });

  it('gets the parent', () => {
    function testName(name: string, expectedPath: string) {
      expect(FileUtils.getParent(normalizeName(name), separator)).toBe(
        normalizeName(expectedPath)
      );
    }

    function testError(name: string) {
      expect(() => FileUtils.getParent(name, separator)).toThrow();
    }
  ...
  it('gets the filename', () => {
    function testName(name: string, expectedName: string) {
      expect(FileUtils.getBaseName(normalizeName(name), separator)).toBe(
        expectedName
      );
    }
  ...
  it('gets the depth', () => {
    function testName(name: string, expectedDepth: number) {
      expect(FileUtils.getDepth(normalizeName(name), separator)).toBe(
        expectedDepth
      );
    }

    function testError(name: string) {
      expect(() => FileUtils.getDepth(name, separator)).toThrow();
    }
  ...
  it('reduces a selection', () => {
    function testReduction(paths: string[], expectedPaths: string[]) {
      expect(
        FileUtils.reducePaths(paths.map(normalizeName), separator)
      ).toEqual(expectedPaths.map(normalizeName));
    }
