describe('emits events when items are created', function () {
  ...
  it('provided the right arguments', function () {
    expect(
      eventListener.onComponentCreated.mostRecentCall.args[0].type
    ).toEqual('component');
    expect(eventListener.onStackCreated.mostRecentCall.args[0].type).toEqual(
      'stack'
    );
    expect(eventListener.onColumnCreated.mostRecentCall.args[0].type).toEqual(
      'column'
    );
    expect(eventListener.onRowCreated.mostRecentCall.args[0].type).toEqual(
      'row'
    );
  });
