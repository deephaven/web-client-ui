...
describe('getHeaderActions', () => {
  ...
  it.each([
    ...
    'shows correct expand/collapse actions when column is expanded=%s',
    (isExpanded, shouldShowExpand) => {
      ...
      asMock(model.isColumnExpandable).mockReturnValue(true);
      asMock(model.isColumnExpanded).mockReturnValue(isExpanded);
      ...
      expect(menuItems.some(a => a.title === 'Expand Column1')).toBe(
        shouldShowExpand
      );
      expect(menuItems.some(a => a.title === 'Collapse Column1')).toBe(
        !shouldShowExpand
      );
    }
  );
